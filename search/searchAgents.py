...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(state[1]) == 4        #if tuple size is 4 then all the corners have been visited
        util.raiseNotDefined()
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            childvalues = (self.getNextState(state,action),action,self.getActionCost(state,action,self.getNextState(state,action)))
            children.append(childvalues)
    ...
    def getNextState(self, state, action):
        ...
        if nextState in self.corners:                           #if next position is a corner
            if nextState not in state[1]:                       #that is not in tuple
                return (nextState, state[1] + (nextState,))     #return next state with new tuple that includes the corner
        
        return (nextState, state[1])                            #else return next state with the tuple as it is
        util.raiseNotDefined()
...
def cornersHeuristic(state, problem):
    ...
    unvisited = []                             #get all the unvisited corners in a list
    for i in corners:
        if i not in state[1]:
            unvisited.append(i)
    ...
    #this will calculate the sum needed to visit all the remaining corners, going the closest one each time
    sum = 0
    position = state[0]
    while(len(unvisited) > 0):
        mindistance, corner = min([(util.manhattanDistance(position ,corner),corner) for corner in unvisited])
        sum = sum + mindistance
        position = corner
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    #previus solution, 3/4 autograder
    """
    food = foodGrid.asList()
    if len(food) == 0:
        return 0
    return  max([util.manhattanDistance(position, food) for food in food])
    """
    food = foodGrid.asList()
    if len(food) == 0:
        return 0                #is goal state.

    #max distance between starting potition and food   
    maxdistance = max([util.manhattanDistance(position, food) for food in food])

    #this calculates the distance between two foods and if its bigger than previus max distance it will replace it
    for i in food:
        for j in food:
            if(i!=j):
                dist = util.manhattanDistance(i,j)
                if(dist > maxdistance):
                    maxdistance = dist
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        complete the problem definition.n
        """
        x,y = state
